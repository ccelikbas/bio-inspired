...
class Aircraft:
    ...
    def update(self, dt, all_aircraft, LOCAL_SEP_RADIUS):
        # Compute my heading vector to next waypoint
        ...
        # Look for any “leader” ahead on the same converging lane 
        COS_THRESH = 0.99   
        for other in all_aircraft:
            ...
            # projection: along > 0 means other is in front
        ...
        # accelerate/decelerate toward target_speed 
...
class GlobalPSO:
    ...
    def _simulate_cost(self, speeds):
        """
        Simulate trajectories under `speeds`, starting from each AC's current position. 
        """
        ...
        for t in samples:
            ...
            for i in range(self.m):
                ...
                # Compute how far this AC already is along its path:
                ...
                # Simulate forward-moving distance at constant speed
                forward = speeds[i] * τ
    ...
    def optimize(self):
        ...
        for i, ac in enumerate(self.acs):
            ...
            # distance up to last passed waypoint
            already = cum[seg]

            # plus the partial distance from that waypoint to current position
            A = pts[seg]
            P = ac.position
            mean_lat = np.deg2rad((A.y + P.y)/2)
            dlat = (P.y - A.y) * M_PER_DEG
            ...
            # remaining = total path length minus what’s already flown
